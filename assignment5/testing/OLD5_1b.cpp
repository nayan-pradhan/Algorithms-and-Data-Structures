...
//functions
long long naiveRecursive (int n);
long long bottomUp (int n);
long long closedForm (int n);
void matrixRep (int n);
void matrixPower (long long matrix[2][2], int n);
void matrixMultiply (long long matrixA[2][2], long long matrixB[2][2]);
...
int main() {
    ...
    while (1) {
        for (int n = 1; n <= userInput; n++) {
            ...
            else if (duration.count() >= maxTime) {
...
long long bottomUp (int n) {
    ...
    long long arr[n]; //an array of n elem is created
...
void matrixMultiply (long long int matrixA[2][2], long long int matrixB[2][2]) {
    ...
    //         auto stopClosed = std::chrono::high_resolution_clock::now();
    //         auto durClosed = std::chrono::duration_cast
    //             <std::chrono::nanoseconds>(stopClosed - startMatrix);
    //         output << durClosed.count() << " ";
    //         if (durClosed.count() >= maxTime) {
    //             break;
    //         }
    //     }
    // }
    ...
    //         auto stopMatrix = std::chrono::high_resolution_clock::now();
    //         auto durMatrix = std::chrono::duration_cast
    //             <std::chrono::nanoseconds>(stopMatrix - startClosed);
    //         output << durMatrix.count() << " ";
    //         if (durMatrix.count() >= maxTime) {
    //             break;
    //         }
    //     }
    // }
